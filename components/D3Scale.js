...
class D3Scale extends Component {
  render() {
    const scalePrice = d3Scale
      .scaleLinear()
      .domain([8200, 8300])
      .range([0, 200]);
    const scalePriceCorrect = d3Scale
      .scaleLinear()
      .domain([8200, 8300])
      .range([0, 200].reverse());
    return (
  ...
  - highest Price: 8300`}
        ...
        <T>
          {`  scalePrice(8210): ${scalePrice(8210)}
  ...
  const scalePriceCorrect =
    ...
    .range([0, 200].reverse());
        ...
        >
          <Path
            d={`M10 ${scalePriceCorrect(8210)} 30 ${scalePriceCorrect(
              8254
            )} 50 ${scalePriceCorrect(8276)}`}
            stroke="#666"
            fill="none"
          />
        </Svg>
