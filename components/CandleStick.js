...
class CandleStick extends Component {
  ...
  getStockQuotes = () => {
    ...
    const url = `http://m.cnyes.com/api/v1/charting/history?symbol=tse:2330&from=${Math.floor(
      from / 1000
  ...
  getLinearScale(domain, range, isTime = false) {
    return (isTime ? d3Scale.scaleTime() : d3Scale.scaleLinear())
      .domain(domain)
      .range(range);
  }
  ...
  handlePress = e => {
    ...
    const current = Math.floor(
      (deviceWidth - locationX) / (barWidth + 2 * barMargin)
    );
  ...
  render() {
    ...
    const priceScale = this.getLinearScale(
      [lowestPrice, highestPrice],
      [0, defaultStockChartHeight].reverse()
    );
        ...
        <Svg height={defaultStockChartHeight} width={deviceWidth}>
          <G onPress={this.handlePress}>
            ...
            {t.map((_, i) => {
              ...
              const [scaleO, scaleC, yTop, yBottom] = [
                item.o,
                item.c,
                item.h,
                item.l
              ...
              const x =
                deviceWidth - barWidth * (i + 1) - barMargin * (2 * i + 1);
              ...
              return (
                <G key={i}>
                  ...
                  {current === i && (
                    <Path
                      stroke="#666"
                      d={`M${x + barWidth / 2} 0 ${x +
                        barWidth / 2} ${defaultStockChartHeight}`}
                      strokeWidth="0.5"
                    />
                  )}
                  {current === i && (
                    <Path
                      stroke="#666"
                      d={`M0 ${scaleC} ${deviceWidth} ${scaleC}`}
                      strokeWidth="0.5"
                    />
                  )}
                </G>
              ...
              priceScale.ticks(10).map((p, i) => {
                return (
                  <G key={i}>
                    ...
                    <Path
                      d={`M0 ${priceScale(p)} ${deviceWidth - 25} ${priceScale(
                        p
                      )}`}
                      stroke="#ddd"
                      strokeWidth="1"
